...
class AdvanceOrderView(View):
    ...
    def get_address(self, uid):
        ...
        for addr in all_address:
            ...
            if addr.is_default:
                address_default.append(add_dict)
            else:
                address_no_default.append(add_dict)
...
#         cart_obj = CartsView()
#         all_data = cart_obj.get_carts_all_data(uid)
...
#             skus = SKU.objects.filter(id__in=carts_dict.keys(), is_launched=True)
#             # 检查库存/修改库存/创建商品订单
#             for sku in skus:
...
#                 result = SKU.objects.filter(id=sku.id, version=old_version).update(stock=sku.stock - carts_count,
