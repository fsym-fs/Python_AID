...
def send_active_email(email_address, v_url):
    ...
    ''' % (v_url)
    send_mail(subject, '', from_email=settings.EMAIL_HOST_USER, recipient_list=[email_address],
...
class AddressView(View):
    # 特点一:对于没有定义动作方法的请求,会直接返回405的请求
    # 特点二:所以该url的请求进入视图类,先走该方法
    @logging_check
    def dispatch(self, request, *args, **kwargs):
        # 所以该url的请求进入视图类,优先走该方法
        # 可以进行集中的参数检查
        print('----dispatch do--')
        json_str = request.body
        if json_str:
...
class OauthWeiboView(View):
    ...
    def get(self, request):
        ...
        # {'access_token': '2.00l1GMDIBBxi_C21e326af80wYlWmD', 'remind_in': '157679999', 'expires_in': 157679999, 'uid': '7376275951', 'isRealName': 'true'}
        # 查询当前微博用户是否第一次登录
        try:
            # print('1')
    ...
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse({'code': 10111, 'error': 'Create user error!'})
