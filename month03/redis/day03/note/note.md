...
​	1.原子性(Atomicity):事务中的SQL语句，要么全部执行,要么全部不执行,可以回滚【转账按例】。

​	2.一致性(Consistency):事务执行之前与执行之后的数据是合法的,数据库的完整性不被破坏。

​	3.隔离线(Isolation):当有多个事务并发执行时,不能相互影响,事务与事务之前是相互隔离的。

​	4.持久性(Durability):事物一旦提交,他对数据库的改变必须是永久的。
...
3.关闭哨兵   sudo /etc/init.d/redis-sentinel stop
```
