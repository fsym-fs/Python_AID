...
def index_view():
    if request.method == 'GET':
        ...
        print(goods_list)
        return render_template('mall/index.html', goods_list=goods_list, news_list=news_list, all_len=all_len)


def search_view():
    if request.method == 'GET':
        good_name = request.args.get('ser_text')
        print(good_name)
        """
        ...
        """
        good_obj = Good.query.filter(Good.inventory>=0,Good.name.like("%" + good_name + "%") if good_name is not None else "")
        print(good_obj.all())
        # return jsonify('ok')
        all_len = len(good_obj.all()) // 9 + 1
        print(all_len)
        try:
        ...
        print('??', page)
        try:
            result = good_obj.filter(Good.is_active == True,Good.inventory>=0).paginate(page, 9)
        ...
        print('good_list')
        print(goods_list)
        return render_template('mall/searchgoods.html', goods_list=goods_list, page=page, all_len=all_len)
...
def all_view():
    if request.method == 'GET':
        # 每页3个，查询第2页的数据
        # ret = paginate(页码，每一页的数据量)
        goods = Good.query.filter(Good.is_active == True,Good.inventory>=0).all()
        all_len = len(goods) // 9 + 1
        try:
            page = int(request.args.get('page', 1))
            # print(page)
        except Exception as e:
        ...
        print('??', page)
        try:
            result = Good.query.filter(Good.is_active == True,Good.inventory>=0).paginate(page, 9)
            print('...', page)
        except Exception as e:
            print('-------')
            print(e)
        ...
        for i in result.items:
            ...
            goods_list.append(lis)
        print(goods_list)
        return render_template('mall/allgoods.html', goods_list=goods_list, page=page, all_len=all_len)
    elif request.method == 'POST':
        # 每页3个，查询第2页的数据
        # ret = paginate(页码，每一页的数据量)
        try:
...
def single_view():
    if request.method == 'GET':
        ...
        lis['pic'] = pic_obj[0].default_pic
        try:
            good_sales_obj = Good_sales.query.filter(Good_sales.gid == good_obj[0].g_id).first()
            lis['sales'] = good_sales_obj.sales_num
        except Exception as e:
            lis['sales'] = 0
        print(lis)
