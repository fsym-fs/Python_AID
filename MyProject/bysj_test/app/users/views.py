...
def send_code_view():
    if request.method == 'GET':
        ...
        try:
            result = send_common_sms(phone, code_str)
            print(result)
            if result['Code'] == 0:
                code = 200
            else:
                code = 1212
            print(result)
            data = {'code': code, 'msg': code_str}
            return jsonify(data)
        except Exception as e:
            print('------msg-----')
            print(e)
            data = {'code': 1211, 'error': '您的手机号不正确!'}
        return jsonify(data)
...
def cart_view():
    if request.method == 'GET':
        ...
        try:
            ...
            print(request_data)
            alipay_view(request_data, name, phone, address)
        except Exception as e:
            pass
        if uname:
            carts_list = []
            try:
                ...
                for item in carts:
                    ...
                    dic['num'] = item.number
                    # print(dic)
                    # a = int(item.number) * int(good.price)
                    print('a', type(dic['price']), float(dic['price']))
                    print('b', type(dic['num']))
                    dic['all_price'] = int(item.number) * float(good.price)
                    # print(dic)
...
def paying_view():
    if request.method == 'GET':
        ...
        if uname and gid_str:
            ...
            try:
                ...
                for item in gid_lis:
                    ...
                    dic['name'] = good.name
                    dic['price'] = good.price
                    # nn = max(Good.inventory,cart.number)
                    good_pic = Good_pic.query.filter(Good_pic.gid == item).first()
                    dic['pic'] = good_pic.default_pic
                    dic['account'] = int(cart.number) * float(good.price)
                    result += int(cart.number) * float(good.price)
                    all_num += cart.number
                    carts_list.append(dic)
                print(carts_list)
            except Exception as e:
        ...
        # print('-------------uname--------------')
        name_en = base64.b64encode(name.encode())
        address_en = base64.b64encode(address.encode())
        # print(oid, result, uname, name_en, phone, address_en)
        ALIPAY_RETURN_URL = 'http://127.0.0.1:5000/cart?uname={}&name={}&phone={}&address={}'.format(uname, name_en,
                                                                                                     phone,
                                                                                                     address_en)
        ALIPAY_NOTIFY_URL = 'http://127.0.0.1:5000/cart?uname={}&name={}&phone={}&address={}'.format(uname, name_en,
                                                                                                     phone,
                                                                                                     address_en)
...
def order_view():
    if request.method == 'GET':
        ...
        try:
            ...
            uid = user.id
            # print(uid)
            all_len = len(Order.query.filter(Order.uid == uid).all()) // 6 + 1
            print(all_len, page)
            try:
            ...
            pass
        print(lis)
        return render_template('users/order.html', lis=lis, page=page, all_len=all_len)


def alipay_view(request_data, name, phone, address):
    result = 0
    # if request.method == 'GET':
    # request_data = {k: request.args[k] for k in request.args.keys()}
    """
    ...
    if result == 1:
        ...
        try:
            ...
            order.o_id = order_id
            # print('---------------namename============')
            name_en = name[2:-1].encode()
            address_en = address[2:-1].encode()
            # print(name_en, address_en)
            # print(type(name_en),type(address_en))
            # print('------------------------')
            name = base64.b64decode(name_en).decode()
            address = base64.b64decode(address_en).decode()
            # print(name, address)
            print("------------uuuuuu-------------")
            order.name = name
            ...
            for i in gid_lis:
                cart = Cart.query.filter(Cart.uid == uid,
                                         Cart.gid == i).first()
                print(cart)
                good = Good.query.filter(Good.g_id == i).first()
                good.inventory = int(good.inventory) - int(cart.number)
                good_sales = Good_sales()
                db.session.add(good_sales)
                good_sales.gid = i
                good_sales.sales_num = int(cart.number)
                db.session.delete(cart)
                db.session.commit()
            db.session.commit()
        except Exception as e:
